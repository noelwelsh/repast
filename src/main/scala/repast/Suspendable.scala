...
enum Suspendable[S, A] {
  ...
  def min(count: Int): Suspendable[S, NonEmptyChain[A]] = {
    assert(
      count > 0,
      s"You tried to create a parser that is repeated at least $count times, but the parser must be repeated 1 or more times."
    )
    Rep(this, count, Int.MaxValue)
  }

  def max(count: Int): Suspendable[S, NonEmptyChain[A]] = {
    assert(
      count > 0,
      s"You tried to create a parser that is repeated at most $count times, but the parser must be repeated 1 or more times."
    )
    Rep(this, 1, count)
  }
    ...
    this match {
      ...
      case r: Rep[s, a] =>
        def repeat(
          ...
          else if count >= max then continuation(accum)(ctrl)
          else
            r.source.loop(
              accum.input,
              accum.offset,
                ...
                (result: Result[a]) =>
                  result match {
                    case Success(a, i, s, o) =>
                      repeat(
                        Success(accum.result :+ a, i, offset, o),
                        count + 1,
                        min,
                        max,
                        ctrl
                    ...
                    case Committed(i, s, o) =>
                      if count < min then continuation(Committed(i, s, o))(ctrl)
                      else continuation(accum)(ctrl)
                    case Epsilon(i, o) =>
                      // If we have reached here we have committed to parsing
                      // some content
                      if count < min then
                        continuation(Committed(i, accum.start, o))(ctrl)
                      else continuation(accum)(ctrl)
                  }
              )
            )(ctrl)
